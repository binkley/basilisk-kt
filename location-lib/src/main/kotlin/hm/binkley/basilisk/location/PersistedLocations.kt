...
class PersistedLocation internal constructor(
    ...
    fun update(block: PersistedMutableLocation.() -> Unit) =
            update(LocationResource(this), block)

    internal inline fun update(
            snapshot: LocationResource?,
            block: PersistedMutableLocation.() -> Unit) = apply {
...
class PersistedMutableLocation internal constructor(
        ...
        private val factory: PersistedLocations) : MutableLocationDetails by record {
    fun save() = apply {
        record.flush()
